...
function validateCred(isValid) {
  // creates a clone of array passed.
...
console.log("Is card number valid?:", validateCred(valid1));
...
//array for invalid card inside batch array
console.log('Invalid card array :', findInvalidCards(batch))

function idInvalidCardCompanies(invalidCards) {
  let cardCompany = [];
  let invalidDigit = findInvalidCards(invalidCards);
  for (let i = 0; i < invalidDigit.length; i++) {
    let firstDigit = invalidDigit[i].slice(0, 1);
    let secondDigit = invalidDigit[i].slice(1, 2);
    if (firstDigit == 3) {
      cardCompany.push("Amex (America Express)");
      ...
      console.log('Not registered on the code');
    }
  }
  return cardCompany;
}
//to check card companies
